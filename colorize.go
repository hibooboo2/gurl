...
func (f *Formatter) Marshal(v interface{}) []byte {
...
func (f *Formatter) pretty(v interface{}, depth int) string {
	...
	default:
		return f.sprintfColor(f.StringColor, `%v`, v)
	}
}
...
func Marshal(v interface{}) []byte {
	return addcolor.Marshal(v)
}

// Format JSON string with default options.
func Format(data []byte) []byte {
	return addcolor.Format(data)
}

func init() {
	addcolor = NewFormatter()
	addcolor.StringColor = color.New(color.FgGreen)
}
